...
void split(AST *a, string& error) {
    string from_id = child(a,2)->text;
    if (exists_tube(from_id)) {
        ...
        if (splitted_length1 == 0) error = from_id + " cannot be splitted because it has length 1";
            ...
            string id1 = child(a,0)->text, id2 = child(a,1)->text;
...
bool declared_error(const string& id, string& error) {
    if (not exists_tube(id)) {
        if (not exists_connector(id)) {
            if (exists_vector(id)) error = id + " is already in use (TUBEVECTOR)";
        }
        else error = id + " is already in use (CONNECTOR)";
    }
    else error = id + " is already in use (TUBE)";
    return failed(error);
}
...
void pop(const string& from_id, const string& to_id, string& error) {
    if (not declared_error(to_id, error)) {
        if (exists_vector(from_id)) {
            Tube top;
...
void set(const string& id, AST *a, string& error) {
    if (not declared_error(id, error)) {
        if (a->kind == "TUBE") {
            ...
            merge(a, id, error);
            if (failed(error)) tubes.erase(id);
        }
        else {
...
void execute(AST *a) {
    if (a == NULL) return;
    string error = "";
    if (a->kind == "=") set(child(a,0)->text, child(a,1), error);
    ...
    else if (a->kind == "SPLIT") split(a, error);
...
/// Operations
ops: assign | parametric_accessor | vector_ops | check | write ;
assign: split | (ID SET^ (ID | tube | connector | vector | merge)) ;
split: LP! ID COMMA! ID RP! SET! SPLIT^ ID ;
